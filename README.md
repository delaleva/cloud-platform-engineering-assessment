...
internet gateway, no NAT, and a route table with no default route. One
interface endpoint for Secrets Manager, giving each subnet a private address
that reaches the service without leaving the VPC.

**Compute.** Two Python 3.13 functions on arm64, both attached to the private
subnets. The API function accepts a POST carrying a JSON message, checks the
caller against an allow list, and returns the message with a timestamp and the
caller's name. The test client function reads its certificate from Secrets
Manager and sends a message to the API.

**API layer.** An internal Application Load Balancer with an HTTPS listener on
port 443 in mutual TLS verify mode, and the API function as its target. Its
security policy is `ELBSecurityPolicy-TLS13-1-2-2021-06`, which accepts TLS 1.3
and 1.2 and refuses anything older. The API is served at `api.internal.cpe`, a
record in a private hosted zone.

**Certificates.** A self-signed certificate authority, generated by Terraform's
`tls` provider, signs two certificates. The server certificate goes into ACM
for the listener to present, so a caller can confirm it reached the right host.
The client certificate goes into Secrets Manager for the test client to
present, so the load balancer knows who is calling. The CA certificate goes
into S3 and from there into the trust store, which is the list of authorities
whose client certificates the load balancer will accept. A copy of it also sits
in the client's secret, so the client can check the load balancer the same way.

**Logging.** Both functions' logs, the VPC flow logs and the Route 53 resolver
query logs go to CloudWatch. The load balancer's access and connection logs go
to S3, which is the only destination it supports.

**Alerting.** An alarm watches server errors returned by the API function
rather than by the load balancer, so it fires only once a caller has got past
the handshake and the allow list and something has then failed. One error in a
minute triggers it. The alarm then publishes a message to an SNS topic, which
passes it on to whatever has subscribed. Nothing has subscribed here; that
would be wired up in production.

**Encryption.** One customer managed key encrypts the secrets, the log groups
and the bucket holding the CA bundle.
...
**2. It resolves the name and connects.** `api.internal.cpe` resolves through
the private hosted zone to addresses inside the VPC, one per availability zone,
and the client opens a connection on port 443. The name requested here is the
name that has to appear in the certificate at the next step.
...
takes the caller's name from the verified subject in the header, checks it
against an allow list read from Secrets Manager, and returns 403 if it is not
there.
...
Each boundary below is a place where something is checked rather than assumed.
If one of them failed, the others would still stop an unauthorised caller,
...
| **The API function** | A caller whose access should have been withdrawn, but whose certificate has not expired | Its verified name is checked against an allow list kept in Secrets Manager, so a caller can be removed without redeploying anything or reissuing a certificate |
...
the two handlers, and debugging. Almost every figure and behaviour in this file
was then checked against AWS documentation or against the deployed environment.

### What needed correcting

Enough that I stopped taking anything on trust. These are the ones I remember.
...
**It invented a reason rather than the real one.** Asked why the CA key is
stored in a secret that nothing reads, it said the CA could then sign again
without being regenerated. The real reason is custody: state is local here, so
that secret is the only other copy, and losing the state file would otherwise
lose the authority behind every certificate.

**It got a charge wrong.** It said a KMS key keeps costing money during its
...
**1. Reusable module structure.** I built it as four modules: `network`,
`certificates`, `api` and `test_client`. The root only calls them and passes
values between them.

**2. VPC Flow Logs and a Route 53 Resolver query log.** I turned both on. Flow
logs answer whether anything in the private subnets reached an address it
should not, and which connections were rejected. The resolver log is the DNS
equivalent: which workload looked up which name, which would catch a function
resolving a hostname it should never need.

**3. Environment separation.** Each environment gets its own directory and its
own state, so applying one cannot touch another. I chose that over workspaces
because every workspace [shares a single backend and one set of
credentials](https://developer.hashicorp.com/terraform/language/state/workspaces).
For anything regulated I would go further and give each environment its own
account, since an over-broad IAM policy can hand out too much inside an account
but cannot reach into another one.

### Considered and decided against

**4. AWS WAF.** A web application firewall inspects incoming HTTP requests and
blocks the ones that look hostile, such as SQL injection or a flood from a
single address. It judges the request because it cannot know the caller. Here
the caller is already known: nothing outside the VPC can reach the load
balancer, and every caller proves its identity with a certificate before a
request exists.
...
**6. Lambda versioning and aliases with a safe deployment strategy.** Today a
deploy replaces the running code immediately, with no way back except another
deploy. I would publish a numbered version each time and point the load
balancer at an alias, so changing which version serves is just moving a
pointer. CodeDeploy can then shift traffic across gradually and roll back on
its own if the server error alarm fires.

**7. Terraform tests.** I would use `terraform test`, which can assert against
a plan or against real infrastructure it creates and then destroys. The point
is to change a module and know nothing that depends on it has broken. With four
modules here, that is what I would want covered.

**8. X-Ray or OpenTelemetry tracing.** I would turn on tracing for both
functions, using OpenTelemetry since AWS has put the X-Ray SDKs into
maintenance mode and [recommends
migrating](https://docs.aws.amazon.com/xray/latest/devguide/xray-services.html).
